...
### Cover images (updated 25 Sep)
Each post gets a unique AI illustration: `backend/cover_art.py` builds a topic-specific scene prompt (SMS, email, follow-up, Zoom/video, LinkedIn, timing, benchmarks, strategy, reminders, software) and calls Gemini image models (`GEMINI_IMAGE_MODELS`, default `gemini-3.1-flash-lite-image`, then `gemini-3.1-flash-image`) at 4:5; the image is stored in `blog_cover_art` and composited on the right of the 1200x630 brand layout (title, topic tag, logo). Generated on publish; `POST /api/seo/covers/refresh` (`{}` = all published posts without art, `{"slugs": [...]}`, `{"force": true}`) back-fills. Gemini image models are paid-only (roughly $0.02-0.07 per image), so the key needs billing; `COVER_ART_ENABLED=false` turns it off. Without art, covers use a topic-matched drawn motif (phone, envelope, video grid, funnel, calendar, bell, bars, seats) and a word-matched topic tag.
